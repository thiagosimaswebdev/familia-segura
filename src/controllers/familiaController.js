...
async function listarFamilias(req, res) {
  try {
    ...
    // ─────────────────────────────────────
    // BUSCA PÚBLICA POR NOME/SOBRENOME
    // Procura:
    // - responsável
    // - membros citados nas observações
    // ─────────────────────────────────────
    if (nome) {
      params.push(`%${nome}%`);
      ...
      condicoes.push(`
        (
          LOWER(f.nome_responsavel) LIKE LOWER($${params.length})
          OR LOWER(f.observacoes) LIKE LOWER($${params.length})
        )
      `);
    }
  ...
  try {
    ...
    if (abrigo_id) {
      ...
      const abrigo = await pool.query(
        `SELECT id, vagas_disponiveis, status
         FROM abrigos
         WHERE id = $1`,
        [abrigo_id]
      );
      ...
      if (abrigo.rows[0].status === "fechado") {
        return res.status(400).json({
          erro: "Abrigo está fechado",
        });
      }

      if (abrigo.rows[0].vagas_disponiveis === 0) {
        return res.status(400).json({
          erro: "Abrigo sem vagas disponíveis",
        });
      }
    ...
    res.status(500).json({
      erro: "Erro ao cadastrar família",
    });
  }
}

// Vincula família a abrigo
async function vincularAbrigo(req, res) {
  try {
    ...
    res.json({
      mensagem: "Família vinculada com sucesso",
      familia: resultado.rows[0],
    });
